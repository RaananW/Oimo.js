...
OIMO.RigidBody = function(X, Y, Z, Rad, Ax, Ay, Az){
	...
	// specify the type you want to set the arguments of setupMass method.
	this.type = 0;
...
OIMO.RigidBody.prototype = {
	...
	removeShape: function(shape){
		var remove = shape, prev = remove.prev, next = remove.next;

		if(remove.parent !== this)
			return;

		// Avoid problems with `next` and `prev`
	 ...
	 * @param tpe: The body type
	 * @param adjustPosition
	 */
	setupMass: function(tpe, AdjustPosition){
		var adjustPosition = (AdjustPosition !== undefined) ? AdjustPosition : true;
		var type = tpe || this.BODY_DYNAMIC;
